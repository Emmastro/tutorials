...
def process_article_data(batch: int):
    ...
    path = f"gs://okapi/article_titles/articles_{batch}.parquet"
    print(path)
    articles_batch = pd.read_parquet(path, storage_options=storage_options)

    print(articles_batch)
    ...
    for chuck, articles in enumerate(np.array_split(articles_batch, 10)):
        ...
        for i, page in enumerate(pages):
            ...
            if title:
                ...
                print(f"Page {i} {page} no content")
